...
eval_df['x6'] = calc_smooth_mean(df1=eval_df,df2=None,cat_name='x6',target='y',weight=WEIGHT)
eval_df['x5'] = calc_smooth_mean(df1=eval_df,df2=None,cat_name='x5',target='y',weight=WEIGHT)
...
eval_df = preprocessing.normalize(eval_df)
#X=X.values
#X = X.astype(float)
...
rf.predict(eval_df)
score = cross_val_score(RandomForestClassifier(n_estimators = 1050),X,y,cv=10)
np.average(score)
